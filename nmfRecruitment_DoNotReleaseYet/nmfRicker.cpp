                        ...
                        std::vector<double> &SpawningBiomass)
{
    ...
    SpawningBiomassValue = m_getSpawningBiomass(yearIdx,Abundance);
    //SpawningBiomass.push_back(SpawningBiomassValue);

    for (int Year = 0; Year < NumYears-1; ++Year) {

        // First get abundance for first age group
        RecruitmentValue = m_getRecruitment(Year+1,SpawningBiomassValue);
        ...
        SpawningBiomassValue = m_getSpawningBiomass(Year+1,Abundance);
//      SpawningBiomass.push_back(SpawningBiomassValue);
    }
...
double
nmfRicker::m_getRecruitment(const int &yearIdx,
...
double
nmfRicker::m_getSpawningBiomass(const int &yearIdx,
