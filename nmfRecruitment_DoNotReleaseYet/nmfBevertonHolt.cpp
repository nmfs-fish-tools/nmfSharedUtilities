...
/*
 * N.B. This implementation seems to be the same as that of Ricker.  Not sure why.
 * Perhaps it was never implemented or it was accidentally copied over.  Regardless,
 * if used again, it (or Ricker) must be fully implmeneted.
 */

nmfBevertonHolt::nmfBevertonHolt(
...
void
nmfBevertonHolt::getAbundance(boost::numeric::ublas::matrix<double> &Abundance,
...
//        m_prevAbundance.push_back(initialAbundance);
    ...
    for (int Year = 0; Year < NumYears-1; ++Year) {
        ...
        for (int Age=0; Age<m_maxAge-m_minAge-1; ++Age) {
            if (useCatch) {
                ...
                Abundance(Year+1,Age+1) =
                        (Abundance(Year,Age)*eFactorAge - m_fisheryCatch(Year,Age)) * eFactorAge;
            } else {
...
double
nmfBevertonHolt::getRecruitment(const int &yearIdx,
                                const double &SpawningBiomass)
{    
/*
//    double sigma = m_sigma[yearIdx];

    return ((m_alpha[m_species] * SpawningBiomass) /
            (m_beta[m_species]  + SpawningBiomass));
...
double
nmfBevertonHolt::getSpawningBiomass(const int &yearIdx,
